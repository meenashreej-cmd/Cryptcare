...
Phase 2 Step 6 audit trail: all three functions (create_claim, list_claims,
update_claim_status) now write AccessLog entries. Previously this was the
only service with zero audit coverage — every claim creation, read, and
status decision was invisible to the audit trail.
"""
...
from app.core.audit import write_access_log as _write_access_log_base
...
def _write_access_log(db, user_id, action, resource_id=None, patient_id=None):
    return _write_access_log_base(
        db, user_id=user_id, resource_type="insurance_claims",
...
def create_claim(db: Session, current_user: CurrentUser, payload: ClaimCreateRequest) -> InsuranceClaim:
    ...
    _write_access_log(db, current_user.id, AccessActionEnum.WRITE, resource_id=claim.claim_id, patient_id=patient_id)
    db.commit()
    db.refresh(claim)
    return claim
...
def list_claims(db: Session, current_user: CurrentUser) -> list[InsuranceClaim]:
    if current_user.role == RoleEnum.PATIENT.value:
        ...
        _write_access_log(db, current_user.id, AccessActionEnum.READ, patient_id=patient_id)
        db.commit()
        return claims
        ...
        claims = (
            db.query(InsuranceClaim)
            .filter(InsuranceClaim.insurer_id == insurer_id)
            .order_by(InsuranceClaim.created_at.desc())
            .all()
        )
        _write_access_log(db, current_user.id, AccessActionEnum.READ)
        db.commit()
        return claims
...
def update_claim_status(
    ...
    if claim.insurer_id != insurer_id:
        # Log the attempted cross-insurer access before rejecting
        _write_access_log(db, current_user.id, AccessActionEnum.DENIED, resource_id=claim_id, patient_id=claim.patient_id)
        db.commit()
    ...
    if claim.status != ClaimStatusEnum.PENDING:
        raise HTTPException(
            status_code=400,
            detail=f"Cannot transition from {claim.status} to {new_status}. Claim is immutable after decision.",
        )
    ...
    patient_user_id = (
        ...
        .filter(PatientProfile.patient_id == claim.patient_id)
        .first()
        .user_id
    )
    db.add(Notification(
    ...
    _write_access_log(db, current_user.id, AccessActionEnum.WRITE, resource_id=claim_id, patient_id=claim.patient_id)
    db.commit()
    db.refresh(claim)
    return claim
